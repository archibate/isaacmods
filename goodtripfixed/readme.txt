...
2026年7月12日
·右上角大地图点击传送针对 MinimapAPI 用户彻底重写：直接向 MinimapAPI 查询每个房间的实际绘制位置，小图/边框图/按TAB展开的大图、任意自定义位置、镜像世界全部精确命中——修复装有 MinimapAPI 时点击偏移或点不中的问题。
·原版右上角地图点击经游戏内逐像素标定，主世界与镜像世界均已精准。
·游戏中途调整 HUD 偏移后，点击判定即刻跟随（此前要到下一层才生效）。
·新增 MCM 点击自校准滑条（CalibClickMain / CalibClickMirror，±17 像素）：若你的配置下点击仍有偏差，自己拖到精准即可——落点偏左调大、偏右调小，无需等 mod 更新。

2026年7月12日（2.2.33）
·修复部分游戏版本上传送报错"StartRoomTransition 参数类型错误"导致传送失效的问题：一处历史遗留的重复过场调用把动画布尔值当动画编号传入，严格校验的版本会当场拒绝（感谢 校医院也有B超 提供控制台报错）。

2026年7月13日（2.2.34）
·修复老忏悔（非忏悔+）上镜像层右上角地图点选偏移两格的问题：忏悔+重新锚定了右上角地图，相位常数现按游戏版本区分（两个版本均经游戏内逐像素标定，感谢 阿泽 反馈）。
·点击自校准滑条量程放宽至 ±100 像素，并新增纵向滑条（CalibClickMainY / CalibClickMirrorY），任何配置下的点击偏移都可自行校准。

2026年7月28日（2.2.35）
·修复“可传送到未清邻居房”漏判连通性的问题：原版要求目标房已探索且已清空，而邻居房规则只看目标是否挨着某个已清房间，于是用皇帝牌跳进 Boss 房这类孤岛后，仍能穿过大片未探索房间传回起始房（新增 MCM: FairTripPath，默认开启，关掉即可恢复自由跳跃。感谢 Kattack 反馈）。