...
-- A blade that only ever strikes as itself is out, yet never behind such a blow.
...
local function weaponOf(source, flags, amount, victim, extraSource)
    ...
    if source.Type == EntityType.ENTITY_KNIFE then
        local subtype = entity ~= nil and entity.SubType or nil
        return knifeName(source.Variant, subtype) or heldWeapon(owner, MELEE_WEAPONS) or "Melee"
    end
...
-- A fart poisons what it reaches, then hurts it as the player carrying no flag and
-- handing nothing over, and only then puts its cloud in the room, all inside one
-- call -- measured on The Bean's, and taken to hold for any other item's. While
...
local function syncStatus(victim, frame)
    ...
    if poisonAppeared then data.dmvpPoisonFrom = applier end
